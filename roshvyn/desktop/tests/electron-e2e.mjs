// Roshvyn Desktop Electron end-to-end check (Playwright Electron driver). Mac run; Windows needs its own run.
// Requires the Desktop services to be running (roshvyn/scripts/mac/start-desktop.sh --services-only).
...
const out = path.join(repo, 'roshvyn/tests/evidence/desktop');
...
const app = await electron.launch({
  ...
  env: {
    ...process.env, ROSHVYN_CLIENT_DATA: client, ROSHVYN_LIBRECHAT_URL: 'http://127.0.0.1:3090', ROSHVYN_APPROVAL_PORT: '3099',
    ROSHVYN_APPROVAL_SECRET_FILE: secretFile, ROSHVYN_WORKSPACE_CONFIG: path.join(os.homedir(), '.roshvyn/workspace.json'),
  },
});
...
try {
  ...
  check('window title is branded', (await app.evaluate(({ BrowserWindow }) => BrowserWindow.getAllWindows().map((w) => w.getTitle()))).some((t) => /Roshvyn/.test(t)));

  // 2. renderer isolation: the LibreChat page gets no preload and no Node
  const iso = await main.evaluate(() => ({ require: typeof window.require, process: typeof window.process, roshvyn: typeof window.roshvyn }));
  check('LibreChat page has no Node access and no Roshvyn API', iso.require === 'undefined' && iso.process === 'undefined' && iso.roshvyn === 'undefined', JSON.stringify(iso));
  ...
  check('status bar shows workspace and model API', /Roshvyn-Workspace/.test(pill) && /Model API: online/.test(pill), pill.replace(/\n/g, ' '));
  const api = await bar.evaluate(() => Object.keys(window.roshvyn));
  ...
  const st = await bar.evaluate(() => window.roshvyn.status());
  ...
  for (const [decision, selector] of [['Deny', '#deny'], ['Approve once', '#approve']]) {
    const t0 = Date.now();
    const pending = postApproval({ command: `wc -c notes.md  # e2e ${decision}`, cwd: 'hello-roshvyn', shell: 'zsh', reason: 'not in allowlist' });
    let dlg = null;
