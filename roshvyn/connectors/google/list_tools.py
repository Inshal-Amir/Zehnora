"""List the tools the pinned Google Workspace MCP exposes with Roshvyn's selection (no Google login needed)."""
...
async def main():
    env = dict(os.environ, GOOGLE_OAUTH_CLIENT_ID="listing-only.apps.googleusercontent.com", GOOGLE_OAUTH_CLIENT_SECRET="listing-only",
               WORKSPACE_MCP_CREDENTIALS_DIR=os.path.expanduser("~/.roshvyn/google-credentials-listing"))
