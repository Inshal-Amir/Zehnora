"""Roshvyn search MCP server: real SearXNG search + a separate public page fetcher.
...
SEARXNG = os.environ.get("ROSHVYN_SEARXNG_URL", "http://127.0.0.1:8888")
MAX_BYTES = 2 * 1024 * 1024
MAX_TEXT = 8000
MAX_REDIRECTS = 5
UA = "RoshvynDesktop/0.1 (+local assistant page reader)"
LOG = Path(os.environ.get("ROSHVYN_SEARCH_LOG", Path.home() / ".roshvyn" / "search-connector.jsonl"))

mcp = FastMCP("roshvyn-search")
...
def main() -> None:
    print(f"roshvyn-search MCP using {SEARXNG}", file=sys.stderr)
    mcp.run()
