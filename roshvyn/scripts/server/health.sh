...
fail=0
for s in postgres vllm litellm platform-api nginx; do
  st="$(docker inspect -f '{{.State.Health.Status}}' "roshvyn-$s-1" 2>/dev/null || docker inspect -f '{{.State.Status}}' "roshvyn-$s-1" 2>/dev/null || echo missing)"
...
docker exec roshvyn-vllm-1 nvidia-smi --query-gpu=memory.used,memory.total --format=csv,noheader 2>/dev/null | sed 's/^/  gpu memory: /' || true
exit $fail
