...
[ -d "$ROSHVYN_MODEL_DIR/$ROSHVYN_MODEL_SUBDIR" ] || die "model not downloaded: run download-model.sh"
[ -f "$REPO/roshvyn/portal/dist/index.html" ] || die "portal not built: cd roshvyn/portal && npm ci && npm run build"
...
say "starting postgres + vllm (model load can take minutes)"; dc up -d postgres vllm
wait_healthy() { local s="$1" i=0; until [ "$(docker inspect -f '{{.State.Health.Status}}' "roshvyn-$s-1" 2>/dev/null)" = healthy ]; do
  i=$((i+1)); [ $i -ge 180 ] && die "$s not healthy (dc logs $s)"; sleep 5; done; say "$s healthy"; }
wait_healthy postgres; wait_healthy vllm
...
say "model deployment: $ROSHVYN_MODEL_REPO @ ${ROSHVYN_MODEL_REVISION:0:12} (vllm ${VLLM_IMAGE_TAG:-v0.29.0}, ctx ${ROSHVYN_MAX_MODEL_LEN:-4096})"
docker exec roshvyn-vllm-1 nvidia-smi --query-gpu=name,memory.used,memory.total --format=csv,noheader || true
