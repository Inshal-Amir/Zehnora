#!/usr/bin/env bash
# Roshvyn Mac development platform: PostgreSQL + model upstream + LiteLLM + platform API + portal.
#
#   roshvyn/scripts/mac/dev-platform.sh start [mock|dev-local-4b]
#   roshvyn/scripts/mac/dev-platform.sh stop
#   roshvyn/scripts/mac/dev-platform.sh status
#
...
DEV_GGUF="${ROSHVYN_DEV_GGUF:-$REPO/.local-dev/models/Qwen3.5-4B-Q4_K_M.gguf}"
...
start_llama() {
  ...
  [ -f "$DEV_GGUF" ] || die "dev model not found at $DEV_GGUF. This script does not download models; set ROSHVYN_DEV_GGUF to an existing small GGUF."
...
start_litellm() { # config
  ...
  (
    ...
    export PATH="$PWD/.venv/bin:$PATH" LITELLM_MASTER_KEY="$(cat "$SECRETS/litellm-master-key")" \
      DATABASE_URL="postgresql://roshvyn_litellm:$lp@127.0.0.1:$PG_PORT/roshvyn_litellm" \
...
seed_catalog() { # profile
  ...
  case "$1" in
    mock) identity="MOCK (roshvyn-mock via LiteLLM, development only - not a model)"; context=4096 ;;
  ...
  (cd "$R/platform-api" && uv run --frozen python -m app.cli seed-model --alias roshvyn-coder --identity "$identity" \
     --description "Roshvyn coding model ($1 profile)" --context "$context" --max-output 1024 --default-output 512 >/dev/null)
  say "model catalog: roshvyn-coder -> $identity"
}
...
start_api() { # profile
  ...
  (cd "$R/platform-api" && uv run --frozen alembic upgrade head >/dev/null 2>&1 \
     && ROSHVYN_PROFILE="$1" nohup uv run --frozen uvicorn app.main:app --host 127.0.0.1 --port "$API_PORT" >>"$LOGS/platform-api.log" 2>&1 & echo $! >"$RUN/platform-api.pid")
...
case "${1:-status}" in
  ...
  stop)
    ...
    say "all Roshvyn dev services stopped (data kept)"
    ;;
  status)
