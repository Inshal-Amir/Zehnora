#!/usr/bin/env bash
# Roshvyn Desktop (Mac): local MongoDB + LibreChat backend/UI + workspace connector, then Electron.
#
#   roshvyn/scripts/mac/start-desktop.sh                  # services + Electron window
#   roshvyn/scripts/mac/start-desktop.sh --services-only  # services only (tests, headless)
#
# Environment (all optional):
#   ROSHVYN_API_BASE     model API base (default http://127.0.0.1:8200/v1; production https://api.<OWNER_DOMAIN>/v1)
#   ROSHVYN_CLIENT_DATA  client state directory (default <repo>/.local-dev/client)
# The customer API key is read from $ROSHVYN_CLIENT_DATA/secrets/roshvyn-api-key (mode 600) or
...
API_BASE="${ROSHVYN_API_BASE:-http://127.0.0.1:8200/v1}"
...
[ -x "$R/connectors/workspace/.venv/bin/python" ] || die "workspace connector env missing: cd roshvyn/connectors/workspace && uv sync"
...
[ -f "$SEC/roshvyn-api-key" ] || [ -n "${ROSHVYN_API_KEY:-}" ] || die "no Roshvyn API key: create one in the portal and save it to $SEC/roshvyn-api-key (chmod 600)"

WS_CONFIG="$HOME/.roshvyn/workspace.json"
mkdir -p "$HOME/.roshvyn"
[ -f "$WS_CONFIG" ] || printf '{"workspace_root": "%s"}\n' "$HOME/Desktop/Roshvyn-Workspace" >"$WS_CONFIG"
...
PY
  ...
  say "Google Workspace connector off (no $SEC/google-client-secret.json; see roshvyn/docs/GOOGLE-SETUP.md)"
fi
...
SEARXNG_DIR="${ROSHVYN_SEARXNG_DIR:-$REPO/.local-dev/tools/searxng}"
...
if alive librechat; then say "LibreChat already running"; else
  ...
  (
    ...
    export HOST=127.0.0.1 PORT=$LC_PORT DOMAIN_CLIENT="http://127.0.0.1:$LC_PORT" DOMAIN_SERVER="http://127.0.0.1:$LC_PORT" \
      MONGO_URI="mongodb://127.0.0.1:$MONGO_PORT/RoshvynDesktop" CONFIG_PATH="$CD/librechat.yaml" ENDPOINTS=agents,custom \
      SEARCH=false NO_INDEX=true APP_TITLE="Roshvyn Desktop" CUSTOM_FOOTER="Roshvyn Desktop · built on LibreChat (MIT)" \
      ...
      ROSHVYN_API_BASE="$API_BASE" NODE_ENV=production TRUST_PROXY=1
    [ -n "${ROSHVYN_API_KEY:-}" ] || export ROSHVYN_API_KEY="$(cat "$SEC/roshvyn-api-key")"
    cd "$REPO"
...
if [ "$SERVICES_ONLY" = 0 ]; then
  [ -d "$R/desktop/node_modules" ] || die "Electron shell not installed: cd roshvyn/desktop && npm install"
  cd "$R/desktop"
  ROSHVYN_CLIENT_DATA="$CD" ROSHVYN_LIBRECHAT_URL="http://127.0.0.1:$LC_PORT" ROSHVYN_APPROVAL_PORT=$APPROVAL_PORT \
    ROSHVYN_APPROVAL_SECRET_FILE="$SEC/approval-secret" ROSHVYN_WORKSPACE_CONFIG="$WS_CONFIG" \
    PATH="$NODE_BIN:$PATH" nohup "$R/desktop/node_modules/.bin/electron" . ${ROSHVYN_ELECTRON_ARGS:-} >>"$LOGS/electron.log" 2>&1 &
  echo $! >"$RUN/electron.pid"
  say "Roshvyn Desktop window starting"
fi
