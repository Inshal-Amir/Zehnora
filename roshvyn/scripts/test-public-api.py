...
Usage (keys come from the environment, never from source):
  export ROSHVYN_BASE_URL=https://api.<OWNER_DOMAIN>/v1      # or http://127.0.0.1:8200/v1 in development
  export ROSHVYN_API_KEY=<customer key>
  uv run --project roshvyn/tests python roshvyn/scripts/test-public-api.py
  uv run --project roshvyn/tests python roshvyn/scripts/test-public-api.py --expect-revoked   # after revoking the key
...
upstream was the development MOCK (header x-roshvyn-mock) so mock runs are never
...
MODEL = os.environ.get("ROSHVYN_MODEL", "roshvyn-coder")
TOOLS = [{
    "type": "function",
    "function": {
        "name": "get_task_count",
...
def main() -> int:
    ...
    args = ap.parse_args()
    base = os.environ["ROSHVYN_BASE_URL"].rstrip("/")
    key = os.environ["ROSHVYN_API_KEY"]
    if base.endswith("/chat/completions"):
        sys.exit("ROSHVYN_BASE_URL must end at /v1, without /chat/completions")
    ...
    def upstream_kind():
        ...
        r.raise_for_status()
        mock = r.headers.get("x-roshvyn-mock") == "true" or r.json()["choices"][0]["message"].get("content", "").startswith("MOCK")
