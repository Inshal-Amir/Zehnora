"""Local administration commands (run on the server, never exposed over HTTP).
  ...
  python -m app.cli seed-model --alias roshvyn-coder --identity "..." --context 4096 --max-output 1024
...
async def create_gateway_key(models: list[str]) -> None:
    secret, token_id = await litellm_client.generate_key(models=models, alias="roshvyn-portal-playground",
                                                         metadata={"purpose": "portal playground (server-side only)"})
    print(secret)
    print(f"(LiteLLM token id {token_id}; store the key above as ROSHVYN_PLAYGROUND_GATEWAY_KEY in server secrets)", file=sys.stderr)
...
def main() -> None:
    ...
    s.add_argument("--alias", default="roshvyn-coder")
    s.add_argument("--identity", required=True)
    s.add_argument("--description", default="Roshvyn coding assistant model")
    ...
    g.add_argument("--models", nargs="+", default=["roshvyn-coder"])
    a = ap.parse_args()
