...
Screenshots go to roshvyn/tests/evidence/portal/ and never show a live key secret.
"""
...
PORTAL = os.environ.get("ROSHVYN_PORTAL", "http://127.0.0.1:5173")
SECRETS = Path.home() / "Desktop/roshvyn/.local-dev/secrets"
...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        r = httpx.post(f"{api_base}/chat/completions", headers={"Authorization": f"Bearer {secret}"},
                       json={"model": "roshvyn-coder", "messages": [{"role": "user", "content": "hello from the portal e2e"}]}, timeout=120)
        check("API call with portal-created key succeeds", r.status_code == 200, f"HTTP {r.status_code}")
        is_mock = r.headers.get("x-roshvyn-mock") == "true" or "MOCK" in r.text
        ...
        page.get_by_placeholder("Message roshvyn-coder").fill("playground e2e hello")
        ...
        r2 = httpx.post(f"{api_base}/chat/completions", headers={"Authorization": f"Bearer {secret}"},
                        json={"model": "roshvyn-coder", "messages": [{"role": "user", "content": "after revoke"}]}, timeout=60)
