...
PGCTL = pathlib.Path.home() / "Desktop/roshvyn/.local-dev/tools/postgres-17.5/bin/pg_ctl"
PGDATA = pathlib.Path.home() / "Desktop/roshvyn/.local-dev/pgdata"
...
mock_before = pathlib.Path.home().joinpath("Desktop/roshvyn/.local-dev/logs/mock.log").read_text().count("POST /v1/chat/completions")
...
try:
    resp = httpx.post(f"{BASE}/v1/chat/completions", headers={"Authorization": f"Bearer {key}"},
                      json={"model": "roshvyn-coder", "messages": [{"role": "user", "content": "should not run"}]}, timeout=60)
...
time.sleep(1)
mock_after = pathlib.Path.home().joinpath("Desktop/roshvyn/.local-dev/logs/mock.log").read_text().count("POST /v1/chat/completions")
