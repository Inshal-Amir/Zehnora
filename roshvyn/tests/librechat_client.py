"""Client for a Roshvyn Desktop LibreChat instance: the same agent chat routes the UI uses.
...
AGENT_NAME = "Roshvyn Coder"
INSTRUCTIONS = """You are Roshvyn Coder, a coding assistant working inside the user's selected workspace folder.
    ...
    def ensure_agent(self, recursion_limit: int = 25, max_context: int = 7000, max_tokens: int = 1024) -> str:
        ...
        if cache.exists():
            try:
                ...
                pass
        agent = self._req("POST", "/api/agents", {**body, "description": "Roshvyn coding agent with workspace tools",
                                                   "provider": "Roshvyn", "model": "roshvyn-coder"})
