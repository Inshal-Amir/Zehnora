...
```bash
cd ~/Desktop/roshvyn
roshvyn/scripts/mac/dev-platform.sh start dev-local-4b    # or: start mock (instant, clearly labelled MOCK)
roshvyn/scripts/mac/start-desktop.sh
```
...
4. **SDK call:** in a terminal:
   ```bash
   export ROSHVYN_BASE_URL=http://127.0.0.1:8200/v1
   export ROSHVYN_API_KEY=<paste the key>
   cd roshvyn/tests && uv run python ../scripts/test-public-api.py
   ```
...
## Part 2: Roshvyn Desktop (about 5 minutes)
...
2. Choose the **Roshvyn Coder** agent. Prompt: `Create a folder named demo-notes with a README.md that explains what Roshvyn is in two sentences, then list the folder.` Show the tool calls, then open the folder in Finder.
...
Replace `ROSHVYN_BASE_URL` with `https://api.<OWNER_DOMAIN>/v1`, run Part 1 from another internet connection, then start Desktop with `ROSHVYN_API_BASE=https://api.<OWNER_DOMAIN>/v1` and run the ABC full-stack task (Gate E).
