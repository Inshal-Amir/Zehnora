# Roshvyn architecture

Two connected products in one repository (LibreChat v0.8.7 at the root, Roshvyn code under `roshvyn/`):

1. **Roshvyn API**: signup, admin-granted credits, customer API keys, usage reporting and an OpenAI-compatible model API running on the university GPU PC.
2. **Roshvyn Desktop**: LibreChat's own UI, backend and agent runtime in an Electron shell, with local connectors (workspace files/commands, web search, Google Workspace) on the user's machine.
...
flowchart TD
    ...
    subgraph Server[University GPU PC: WSL2 + Docker]
        ...
        P <--> D[(PostgreSQL roshvyn_platform)]
        P --> K[LiteLLM internal]
        K <--> Q[(PostgreSQL roshvyn_litellm)]
...
| Data | Store |
|---|---|
| Platform accounts, sessions, wallets, append-only ledger, API-key metadata (HMAC fingerprint, never the secret), usage, playground chats, audit | PostgreSQL `roshvyn_platform` (GPU PC) |
| LiteLLM virtual keys and its own spend logs | PostgreSQL `roshvyn_litellm` (separate role/database; accessed only through LiteLLM's API) |
...
| Roshvyn API key used by Desktop | OS-encrypted file (Electron `safeStorage`), passed only to the LibreChat service process |
