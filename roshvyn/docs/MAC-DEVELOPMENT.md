...
| Python envs | `uv sync` in `roshvyn/platform-api`, `infra/litellm`, `infra/mock-inference`, `connectors/*`, `tests` |
...
```bash
cd ~/Desktop/roshvyn
roshvyn/scripts/mac/dev-platform.sh start mock           # platform + LiteLLM + MOCK model + portal
roshvyn/scripts/mac/dev-platform.sh start dev-local-4b   # same, with the local Qwen3.5-4B (CPU) instead of the mock
roshvyn/scripts/mac/dev-platform.sh status | stop
roshvyn/scripts/mac/start-desktop.sh                     # Desktop: MongoDB + SearXNG + LibreChat + Electron
roshvyn/scripts/mac/start-desktop.sh --services-only
roshvyn/scripts/mac/stop-desktop.sh
```
...
- Desktop LibreChat login: `.local-dev/client/secrets/desktop-account.txt`. Its Roshvyn key: `.local-dev/client/secrets/roshvyn-api-key`.
...
## Tests (Mac)
```bash
cd roshvyn/platform-api && uv run pytest              # Gate A platform suite (needs the mock profile running)
...
ROSHVYN_BASE_URL=http://127.0.0.1:8200/v1 ROSHVYN_API_KEY=... uv run python ../scripts/test-public-api.py
cd ../desktop && node tests/electron-e2e.mjs          # Electron shell (Desktop services running)
```
Set `PLAYWRIGHT_BROWSERS_PATH=~/Desktop/roshvyn/.local-dev/playwright-browsers` for the Playwright-based tests.
