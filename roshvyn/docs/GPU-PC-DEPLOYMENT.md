# Roshvyn on the university GPU PC (step by step)
...
> **Status:** these steps were written and syntax-checked on the development Mac. They have **not been run on the GPU PC yet**. Record the real output of each step in `roshvyn/docs/STATUS.md`.
...
```powershell
powershell -ExecutionPolicy Bypass -File roshvyn\scripts\windows\Preflight.ps1 -GpuContainerTest
```
...
cd ~ && git clone https://github.com/<OWNER>/roshvyn.git roshvyn
cd ~/roshvyn && git checkout demo-v0.1.0            # the tested release tag
```
Keep the repository in `~/roshvyn` (Linux filesystem), not under `/mnt/c`, for speed.

## 3. Create the server secrets
**WSL**:
```bash
cd ~/roshvyn && roshvyn/scripts/server/init-secrets.sh
nano .server-secrets/server.env      # set OWNER_DOMAIN and ROSHVYN_MODEL_DIR (e.g. /home/<you>/roshvyn-models)
```
...
Put the verified value in `ROSHVYN_TOOL_PARSER` in `server.env`.

## 5. Download the model (pinned revision, outside Git)
**WSL**:
```bash
cd ~/roshvyn && roshvyn/scripts/server/download-model.sh
```
Uses `ROSHVYN_MODEL_REPO` + `ROSHVYN_MODEL_REVISION` (full commit sha) from `server.env`, writes `SHA256SUMS` and `ROSHVYN-PROVENANCE.txt` next to the weights. Record both in `docs/MODEL-EVALUATION.md`.

## 6. Build the portal
**WSL** (Node 24 via nvm or the official tarball):
```bash
cd ~/roshvyn/roshvyn/portal && npm ci && npm run build
```

## 7. Start the services (local only, no tunnel yet)
**WSL**:
```bash
cd ~/roshvyn && roshvyn/scripts/server/start.sh
roshvyn/scripts/server/health.sh
```
...
roshvyn/scripts/server/bootstrap-admin.sh admin@<OWNER_DOMAIN>
# model catalog entry shown to customers (tested context/output limits)
docker exec roshvyn-platform-api-1 python -m app.cli seed-model --alias roshvyn-coder \
...
docker exec roshvyn-platform-api-1 python -m app.cli create-gateway-key
nano .server-secrets/platform.env    # ROSHVYN_PLAYGROUND_GATEWAY_KEY=<printed key>; ROSHVYN_PUBLIC_API_BASE=https://api.<OWNER_DOMAIN>/v1
docker compose -f roshvyn/infra/server/compose.yaml --env-file .server-secrets/server.env up -d platform-api
```
...
2. **WSL**: `ROSHVYN_BASE_URL=http://api.<OWNER_DOMAIN>:8080/v1 ROSHVYN_API_KEY=<key> uv run --project roshvyn/tests python roshvyn/scripts/test-public-api.py`
...
4. **WSL**: `roshvyn/scripts/server/start.sh --with-tunnel`
...
```bash
export ROSHVYN_BASE_URL=https://api.<OWNER_DOMAIN>/v1
export ROSHVYN_API_KEY=<customer key>
uv run --project roshvyn/tests python roshvyn/scripts/test-public-api.py
# revoke the key in the portal, then:
uv run --project roshvyn/tests python roshvyn/scripts/test-public-api.py --expect-revoked
```
Also confirm `https://api.<OWNER_DOMAIN>/key/generate` and `https://console.<OWNER_DOMAIN>/v1/models` return 404.

## 12. Connect Roshvyn Desktop on the Mac
**Mac Terminal**:
```bash
ROSHVYN_API_BASE=https://api.<OWNER_DOMAIN>/v1 roshvyn/scripts/mac/start-desktop.sh
```
Use an owner account key with admin-granted credits (admin role does not mean free usage).

## Operations
- **Stop** (keeps data): `roshvyn/scripts/server/stop.sh`. Never run `docker compose down -v`.
- **Backup**: `roshvyn/scripts/server/backup.sh`, then encrypt the folder (`age`/`gpg`) before copying it off the PC. Practise a restore into a separate test database first.
