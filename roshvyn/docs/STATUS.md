...
Last update: 2026-09-22, development Mac (Intel i7-9750H, macOS 26.6.2). Repository `~/Desktop/roshvyn`, branch `feat/platform` (local only).

## Part A: API platform
| Item | State | Evidence |
|---|---|---|
| Repo from LibreChat v0.8.7 with full upstream history; `roshvyn/` namespace; secrets/DBs/models git-ignored | DONE | `git log`, `.gitignore` |
| PostgreSQL (separate `roshvyn_platform` / `roshvyn_litellm` DBs and roles) | DONE (Mac, native 17.5) | versions.md |
...
## Part B: Roshvyn Desktop
| Item | State | Evidence |
|---|---|---|
| LibreChat client profile (Roshvyn API as only model endpoint, customer key in service env only), client MongoDB | DONE (Mac) | `infra/client/librechat.yaml.template` |
...
1. **Create the private GitHub repository** (e.g. `roshvyn`) and share the URL, or log in with `gh`, so `origin` can be added and `main` + tag `demo-v0.1.0` pushed.
...
- Merge `feat/platform` into `main` and tag `demo-v0.1.0` once reviewed.
