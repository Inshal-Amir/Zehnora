...
| Item | Value |
|---|---|
| Upstream | https://github.com/danny-avila/LibreChat (remote `upstream`; partial clone `--filter=blob:none`, full commit history kept) |
...
| origin | not configured yet (private repository to be created by the owner) |
...
| FastAPI / SQLAlchemy / Alembic / psycopg | 0.141.1 / 2.0.54 / 1.20.0 / 3.3.6 | roshvyn/platform-api/uv.lock |
| pwdlib (Argon2id) | 0.3.1 | uv.lock |
| LiteLLM proxy | 1.102.0 + prisma 0.15.0 (Prisma client generated from LiteLLM's schema) | roshvyn/infra/litellm/uv.lock |
| Mock inference | local FastAPI app (development only, labelled MOCK) | roshvyn/infra/mock-inference |
