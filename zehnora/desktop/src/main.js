...
// - Stores the Zehnora API key with OS-backed encryption (safeStorage) and passes it only to the service process.
...
const BRAND = JSON.parse(fs.readFileSync(path.join(REPO, 'zehnora/brand/brand.json'), 'utf8'));
const LIBRECHAT_URL = process.env.ZEHNORA_LIBRECHAT_URL || 'http://127.0.0.1:3090';
const CLIENT_DATA = process.env.ZEHNORA_CLIENT_DATA || path.join(REPO, '.local-dev/client');
const APPROVAL_PORT = Number(process.env.ZEHNORA_APPROVAL_PORT || 3099);
const APPROVAL_SECRET_FILE = process.env.ZEHNORA_APPROVAL_SECRET_FILE || path.join(CLIENT_DATA, 'secrets/approval-secret');
const WORKSPACE_CONFIG = process.env.ZEHNORA_WORKSPACE_CONFIG || path.join(os.homedir(), '.zehnora/workspace.json');
const API_BASE = process.env.ZEHNORA_API_BASE || 'http://127.0.0.1:8200/v1';
const PORTAL_URL = process.env.ZEHNORA_PORTAL_URL || 'http://127.0.0.1:5173';
const APPROVAL_TIMEOUT_MS = 120_000;
const LOCAL_ORIGIN = new URL(LIBRECHAT_URL).origin;

let mainWindow = null;
const trace = (...a) => console.log(`[zehnora ${new Date().toISOString()}]`, ...a);
const pendingApprovals = new Map();

// ---------- helpers ----------
const keyFile = () => path.join(app.getPath('userData'), 'zehnora-api-key.bin');
...
function readApiKey() {
  ...
  const devFile = path.join(CLIENT_DATA, 'secrets/zehnora-api-key');
...
async function ensureServices(splash) {
  ...
  const [cmd, args] = process.platform === 'win32'
    ? ['powershell.exe', ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', path.join(REPO, 'zehnora', 'scripts', 'windows', 'Start-Desktop.ps1'), '-ServicesOnly']]
    : ['bash', [path.join(REPO, 'zehnora/scripts/mac/start-desktop.sh'), '--services-only']];
  splash?.webContents.send('status', 'Starting local services (database, LibreChat, workspace connector)…');
  await new Promise((resolve) => {
    const child = spawn(cmd, args, {
      env: { ...process.env, ZEHNORA_API_KEY: key, ZEHNORA_API_BASE: API_BASE, ZEHNORA_CLIENT_DATA: CLIENT_DATA },
...
// Main window = a thin Zehnora status bar view (local HTML + narrow preload) above the LibreChat view.
// The LibreChat view has NO preload: the chat page cannot reach any Zehnora/Electron API.
...
ipcMain.handle('zehnora:status', () => status());
ipcMain.handle('zehnora:choose-workspace', async () => {
  const r = await dialog.showOpenDialog(mainWindow, {
    title: 'Choose the workspace folder Zehnora may use', properties: ['openDirectory', 'createDirectory'],
...
ipcMain.handle('zehnora:open-portal', () => shell.openExternal(PORTAL_URL));
ipcMain.handle('zehnora:save-key', (_e, key) => {
  const k = String(key || '').trim();
  if (!/^sk-[A-Za-z0-9_-]{16,}$/.test(k)) throw new Error('That does not look like a Zehnora API key (sk-…).');
  saveApiKey(k);
  return true;
});
