...
On the GPU PC this job is done by nginx (zehnora/infra/server/nginx/zehnora.conf.template).
...
Blocked:  everything else (/platform/v1/**, admin, anything unknown) -> 404
  ...
  uv run --project zehnora/platform-api python zehnora/infra/edge-proxy.py            # LAN: 0.0.0.0:8300
  uv run --project zehnora/platform-api python zehnora/infra/edge-proxy.py --host 127.0.0.1
"""
...
app = FastAPI(title="Zehnora edge proxy", docs_url=None, redoc_url=None, openapi_url=None)


@app.get("/healthz")
async def healthz():
    return {"status": "ok", "proxy": "zehnora-edge", "allows": ["GET /v1/models", "POST /v1/chat/completions"]}
...
if __name__ == "__main__":
    ...
    a = ap.parse_args()
    print(f"Zehnora edge proxy on {a.host}:{a.port} -> {UPSTREAM} (only /v1/models and /v1/chat/completions)")
