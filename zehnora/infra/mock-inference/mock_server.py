...
`x-zehnora-mock: true`. It exists to exercise credits, errors and SSE in the
...
MODEL_ID = "zehnora-mock"
HEADERS = {"x-zehnora-mock": "true"}
app = FastAPI(title="Zehnora MOCK inference (development only)")
...
@app.get("/v1/models")
async def models():
    return JSONResponse({"object": "list", "data": [{"id": MODEL_ID, "object": "model", "owned_by": "zehnora-mock"}]}, headers=HEADERS)
