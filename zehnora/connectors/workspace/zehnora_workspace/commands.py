...
- Everything else (unknown programs, shell operators, git push, deletes...) needs an explicit user
  approval from Zehnora Desktop's approval dialog (loopback bridge + per-install secret). With no
  approval UI connected, such commands are refused.
- Child processes get a scrubbed environment (no Zehnora/LiteLLM/cloud credentials).
...
MAX_TIMEOUT = 900
SHELL_META = set("|&;<>`$(){}\n")
SECRET_ENV_PREFIXES = ("ZEHNORA_", "LITELLM_", "OPENAI_", "ANTHROPIC_", "AWS_", "AZURE_", "GOOGLE_", "GH_", "GITHUB_",
...
def data_dir() -> Path:
    d = Path(os.environ.get("ZEHNORA_CONNECTOR_DATA", Path.home() / ".zehnora" / "workspace-connector"))
...
def request_approval(command: str, cwd: str, shell: str, reason: str) -> bool:
    url = os.environ.get("ZEHNORA_APPROVAL_URL")
    secret_file = os.environ.get("ZEHNORA_APPROVAL_SECRET_FILE")
    if not url or not secret_file:
        raise ToolError(f"This command needs user approval ({reason}), but no Zehnora Desktop approval window is connected. "
