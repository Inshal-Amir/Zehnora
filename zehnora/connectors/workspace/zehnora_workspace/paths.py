...
def config_path() -> Path:
    return Path(os.environ.get("ZEHNORA_CONNECTOR_CONFIG", Path.home() / ".zehnora" / "workspace.json"))


def workspace_root() -> Path:
    """Explicitly selected workspace: env var, then the Desktop app's config file, then the default demo root.
    Re-read on every call so switching folders in Zehnora Desktop applies immediately."""
    chosen = os.environ.get("ZEHNORA_WORKSPACE")
    if not chosen:
        try:
            chosen = json.loads(config_path().read_text()).get("workspace_root")
        except (OSError, ValueError):
            chosen = None
    root = Path(chosen).expanduser() if chosen else desktop_dir() / "Zehnora-Workspace"
