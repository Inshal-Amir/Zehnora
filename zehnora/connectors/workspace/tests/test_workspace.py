...
def ws(tmp_path, monkeypatch):
    root = tmp_path / "Zehnora-Workspace"; root.mkdir()
    outside = tmp_path / "outside"; outside.mkdir(); (outside / "secret.txt").write_text("OUTSIDE-SECRET")
    monkeypatch.setenv("ZEHNORA_WORKSPACE", str(root))
    monkeypatch.setenv("ZEHNORA_CONNECTOR_DATA", str(tmp_path / "data"))
    monkeypatch.delenv("ZEHNORA_APPROVAL_URL", raising=False)
    from zehnora_workspace import server
...
def test_policy_classification(ws):
    from zehnora_workspace.commands import classify
    _, root, outside = ws
...
def test_run_command_autonomous_and_env_scrubbed(ws, monkeypatch):
    s, root, _ = ws
    monkeypatch.setenv("ZEHNORA_API_KEY", "sk-must-not-leak")
    (root / "show_env.py").write_text("import os,sys; print(sys.version_info[0]); print('LEAK' if 'ZEHNORA_API_KEY' in os.environ else 'CLEAN')")
...
def test_approval_bridge_approve_and_deny(ws, tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ZEHNORA_APPROVAL_URL", f"http://127.0.0.1:{srv.server_port}/approve")
    monkeypatch.setenv("ZEHNORA_APPROVAL_SECRET_FILE", str(tmp_path / "secret"))
