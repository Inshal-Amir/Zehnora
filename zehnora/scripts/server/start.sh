...
[ -d "$ZEHNORA_MODEL_DIR/$ZEHNORA_MODEL_SUBDIR" ] || die "model not downloaded: run download-model.sh"
...
say "model deployment: $ZEHNORA_MODEL_REPO @ ${ZEHNORA_MODEL_REVISION:0:12} (vllm ${VLLM_IMAGE_TAG:-v0.29.0}, ctx ${ZEHNORA_MAX_MODEL_LEN:-4096})"
docker exec zehnora-vllm-1 nvidia-smi --query-gpu=name,memory.used,memory.total --format=csv,noheader || true
