#!/usr/bin/env bash
# Download the configured model snapshot at its PINNED revision to ZEHNORA_MODEL_DIR (outside git), then
...
uvx --from 'huggingface_hub==1.*' hf download "$ZEHNORA_MODEL_REPO" --revision "$ZEHNORA_MODEL_REVISION" --local-dir "$DEST" \
