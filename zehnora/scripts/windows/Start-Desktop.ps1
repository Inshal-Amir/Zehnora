<#
.SYNOPSIS
  Zehnora Desktop on Windows: local MongoDB + LibreChat + connectors, then the Electron window.
  PowerShell equivalent of zehnora/scripts/mac/start-desktop.sh. Run from the repository root:
    powershell -ExecutionPolicy Bypass -File zehnora\scripts\windows\Start-Desktop.ps1 [-ServicesOnly]
  Prerequisites (listed honestly; not bundled): Git, Node 24.16.0, uv, MongoDB Community (mongod.exe),
  LibreChat built (npm ci; npm run frontend), connector envs (uv sync in zehnora\connectors\*),
  zehnora\desktop npm install. The API key is read from %APPDATA%\Zehnora\secrets\zehnora-api-key
...
#>
param([switch]$ServicesOnly, [string]$ApiBase = $(if ($env:ZEHNORA_API_BASE) { $env:ZEHNORA_API_BASE } else { 'http://127.0.0.1:8200/v1' }))

$ErrorActionPreference = 'Stop'
$Repo = (Resolve-Path (Join-Path $PSScriptRoot '..\..\..')).Path
$R = Join-Path $Repo 'zehnora'
$Data = if ($env:ZEHNORA_CLIENT_DATA) { $env:ZEHNORA_CLIENT_DATA } else { Join-Path $env:APPDATA 'Zehnora' }
...
# Workspace: the real Desktop known folder (follows OneDrive redirection) + Zehnora-Workspace
$wsConfigDir = Join-Path $env:USERPROFILE '.zehnora'; New-Item -ItemType Directory -Force -Path $wsConfigDir | Out-Null
$wsConfig = Join-Path $wsConfigDir 'workspace.json'
if (-not (Test-Path $wsConfig)) {
  $desktop = [Environment]::GetFolderPath('Desktop')
  @{ workspace_root = (Join-Path $desktop 'Zehnora-Workspace') } | ConvertTo-Json | Set-Content -Encoding utf8 $wsConfig
}
...
$yaml = $yaml.Replace('__REPO__/zehnora/connectors/workspace/.venv/bin/python', (& $fwd "$R\connectors\workspace\.venv\Scripts\python.exe"))
$yaml = $yaml.Replace('__REPO__/zehnora/connectors/search/.venv/bin/python', (& $fwd "$R\connectors\search\.venv\Scripts\python.exe"))
...
} else { Say 'port 27028 already in use (assuming Zehnora mongod)' }
...
if (-not (Test-Port 3090)) {
  ...
  $vars = @{ HOST = '127.0.0.1'; PORT = '3090'; DOMAIN_CLIENT = 'http://127.0.0.1:3090'; DOMAIN_SERVER = 'http://127.0.0.1:3090';
             MONGO_URI = 'mongodb://127.0.0.1:27028/ZehnoraDesktop'; CONFIG_PATH = (Join-Path $Data 'librechat.yaml'); ENDPOINTS = 'agents,custom';
             SEARCH = 'false'; APP_TITLE = 'Zehnora Desktop'; ALLOW_REGISTRATION = 'false'; ALLOW_SOCIAL_LOGIN = 'false';
             ALLOW_UNVERIFIED_EMAIL_LOGIN = 'true'; ZEHNORA_API_BASE = $ApiBase; NODE_ENV = 'production' }
  $vars.GetEnumerator() | ForEach-Object { [Environment]::SetEnvironmentVariable($_.Key, $_.Value, 'Process') }
  if (-not $env:ZEHNORA_API_KEY) { $env:ZEHNORA_API_KEY = (Get-Content -Raw (Join-Path $Sec 'zehnora-api-key')).Trim() }
...
if (-not $ServicesOnly) {
  $env:ZEHNORA_CLIENT_DATA = $Data; $env:ZEHNORA_APPROVAL_SECRET_FILE = $approvalSecret; $env:ZEHNORA_WORKSPACE_CONFIG = $wsConfig
  ...
  $p.Id | Set-Content (Join-Path $Run 'electron.pid'); Say 'Zehnora Desktop window starting'
}
